...
class CrossConcatLPD(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(1, self.n_iter):
            ...
            # Dual Cross Attentive concatenation
            sino_concat = self._to3DD(
                self._checkpoint(
                    self.dual_cabs[i - 1],
                    [self._to2DD(sino) for sino in sino_list],
                )
            )

            sino_step += self._to3DD(self.dual_layers[i](self._to2DD(sino_concat)))
            ...
            img_concat = self._to3DP(
                self._checkpoint(
                    self.primal_cabs[i - 1],
                    [self._to2DP(img) for img in img_list],
                )
            )

            img_step += self._to3DP(self.primal_layers[i](self._to2DP(img_concat)))
